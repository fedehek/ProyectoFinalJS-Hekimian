const productos = [
  {
    ...
    img: "imagenes/siranushMdurazno.jpeg",
  },
  {
    id: 02,
    nombre: "Lemoncello",
...
function renderizar(array) {
  ...
  for (const producto of array) {
    ...
    tarjetaBody.innerHTML = `
        <div class="div-img" ><img class="thumbnail" src="${
          producto.img
        }"></div>
        <div class="box-element product">
            <h6><strong>${producto.nombre}</strong></h6>
            <h6 class= "precio"><strong>Precio: $ ${producto.precio.toFixed(
              2
            )}</strong></h6><hr>
            <button id ="${
              producto.id
            }" class="btn btn-outline-secondary add-btn update-cart">Agregar al carrito</button>
           
        </div>
        `;
    contenedor.append(tarjetaBody);
  }
  let agregarCarrito = document.getElementsByClassName(
    "btn btn-outline-secondary add-btn update-cart"
  );
...
function renderizarCarro(array) {
  ...
  for (let producto of array) {
    ...
    cart.innerHTML = `
        ...
        <img id="${
          producto.id
        }" class="chg-quantity update-cart " src="imagenes/mas.png">
        <img id="${
          producto.id
        }" class="chg-quantity-2 update-cart" src="imagenes/eliminar.png">
        </div>
        </div>
...
function totalRender(array) {
  ...
  totalResumen.innerHTML = `
    <h5>Productos: <strong>${unidades} </strong></h5>
...
function totalRenderVacio(array) {
  ...
  totalResumen.className = "total";
  totalResumen.innerHTML = `
    <h5>Productos seleccionados: <strong>  0 </strong></h5>
